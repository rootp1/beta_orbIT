    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-200 text-center max-w-md">
          ...
          <p className="text-gray-600 mb-6">You need to connect your wallet to access the developer dashboard.</p>
          <Link href="/">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-200">
          {isLoading ? (
            ...
            <div className="space-y-4">
              {apps.map((app) => (
                <div key={app.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
                  <div className="mb-4 sm:mb-0 flex-1">
                    <p className="font-bold text-lg text-gray-900">{app.name}</p>
                    {app.description && (
                      <p className="text-sm text-gray-600 mt-1 mb-2">{app.description.length > 100 ? app.description.substring(0, 100) + '...' : app.description}</p>
                    )}
